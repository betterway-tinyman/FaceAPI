...
(function (samchon) {
    ...
    (function (std) {
        ...
        var Vector = (function (_super) {
            ...
            Vector.prototype.insert = function (first, second, third) {
                if (third === void 0) { third = null; }
                if (second instanceof std.Iterator == false && third == null) {
                    return this.insert(first, 1, second);
                }
                else if (second instanceof std.Iterator == false && third != null) {
                    var where = first;
                    var size = second;
                    var val = third;
                    var spliced = this.data_.splice(where.getIndex());
                    var inserts = [];
                    for (var i = 0; i < size; i++)
                        inserts.push(val);
                    (_a = this.data_).push.apply(_a, spliced);
                    (_b = this.data_).push.apply(_b, inserts);
                    return new VectorIterator(this, where.getIndex() + inserts.length);
                }
                else if (third != null && third instanceof std.Iterator) {
                    var myEnd = first;
                    var begin = second;
                    var end = third;
                    var spliced = this.data_.splice(where.getIndex());
                    ...
                    (_c = this.data_).push.apply(_c, spliced);
                    (_d = this.data_).push.apply(_d, inserts);
            ...
            Vector.prototype.erase = function (begin, end) {
                ...
                if (end == null)
                    ...
                    this.data_.splice(startIndex, end.getIndex() - startIndex);
    ...
    (function (library) {
        ...
        var XML = (function (_super) {
            ...
            function XML(str) {
                if (str === void 0) { str = ""; }
                _super.call(this);
                this.properties = new samchon.std.UnorderedMap();
                this.value = "";
                if (str.indexOf("<") == -1)
                    return;
                var start;
            ...
            XML.prototype.parseValue = function (str) {
                ...
                if (end_slash < end_block || end_slash + 1 == str.lastIndexOf("<")) {
                    ...
                    this.value = "";
                    return new samchon.std.Pair(str, false);
                }
                ...
                if (str.indexOf("<") == -1)
                    ...
                    this.value = "";
                return new samchon.std.Pair(str, true);
            };
            /**
    ...
    (function (library) {
        ...
        var CaseGenerator = (function () {
            ...
            CaseGenerator.prototype.at = function (index) {
                throw new samchon.std.AbstractMethodError("Don't create CaseGenerator directly.");
            };
...
(function (samchon) {
    ...
    (function (std) {
        ...
        var UnorderedSet = (function (_super) {
            ...
            UnorderedSet.prototype.insert = function (first, second, third) {
    ...
    (function (protocol) {
        ...
        var ServerConnector = (function () {
            ...
            ServerConnector.prototype.handleReply = function (event) {
                ...
                var invokeArray;
                var indexPair = null;
                var sizePair = new samchon.std.Pair(0, 0);
                var startIndex = 0;
                var endIndex = 0;
                while (true) {
                    var iPair = new samchon.std.Pair(this.str.indexOf("<invoke", startIndex), this.str.indexOf("</invoke>", startIndex)); //FIND WORDS
                    if (iPair.first != -1)
                        sizePair.first++;
                    if (iPair.second != -1)
                        sizePair.second++; //AND COUNTS
                    if (indexPair == null && sizePair.first == 1)
                        indexPair = new samchon.std.Pair(iPair.first, -1); //SPECIFY THE STARTING INDEX
